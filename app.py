...
class Question(db.Model):
    ...
    user = db.relationship("User", backref=db.backref("questions", lazy=True))  # <== ADD THIS
...
def get_notifications():
    ...
    # Fetch latest notifications (e.g., messages from founder, replies, top downloads)
    founder_messages = FounderMessage.query.order_by(FounderMessage.timestamp.desc()).limit(5).all()
    replies = Answer.query.filter(Answer.user_id == user.id).order_by(Answer.created_at.desc()).limit(5).all()
    ...
    for reply in replies:
        question = Question.query.get(reply.question_id)
        if question:
            notifications.append({"message": f"Reply to your question: {reply.answer_text}"})
